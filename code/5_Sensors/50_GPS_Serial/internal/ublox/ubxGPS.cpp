...
// This is an override of the NMEA:decode when ubx is being used
ubloxGPS::decode_t ubloxGPS::decode( char c )
{
    decode_t res = DECODE_CHR_OK;
    uint8_t chr = c;

    //   Serial.printf("DONE class=%02X id=%02X interval=%d\n",
    // (uint8_t)m_rx_msg.msg_class,
    // (uint8_t)m_rx_msg.msg_id,
    // intervalCompleted());
      ...
      case UBX_HEAD:
          ...
          switch (chrCount++) {
            case 0:
              ...
              break;
            case 1:                                    // UBX Packet ID is here. This should be used to map update time
...
//trace << hex << chr;
      ...
      case UBX_CRC_B:
          if (chr != m_rx_msg.crc_b) {
            ...
            // Serial.println("decode completed");
          }
          rxState = (rxState_t) UBX_IDLE;
          break;
...
//}
    ...
    if (res == DECODE_COMPLETED) {
      if (nmeaMessage == (nmea_msg_t)UBX_MSG) {
        completed_packet_meta.is_ubx = 1;
        completed_packet_meta.msg_class = (uint8_t)rx().msg_class;
        completed_packet_meta.msg_id = (uint8_t)rx().msg_id;
        completed_packet_meta.payload_length = rx().length;
      } else {
        completed_packet_meta.is_ubx = 0;
        completed_packet_meta.msg_class = 0;
        completed_packet_meta.msg_id = (uint8_t)nmeaMessage;
        completed_packet_meta.payload_length = 0;
      }
    }
...
bool ubloxGPS::send_baud(uint32_t baud, uint8_t port_id)
{
  uint8_t pkt[28] = {0};
  ...
  pkt[2] = 0x06; // CFG
  pkt[3] = 0x00; // PRT
  pkt[4] = 0x14; // payload length = 20
  pkt[5] = 0x00;

  // Payload
  pkt[6]  = port_id; // UART1 = 1
  pkt[7]  = 0x00;    // reserved0
  ...
  pkt[14] = (uint8_t)((baud >> 0)  & 0xFF);
  pkt[15] = (uint8_t)((baud >> 8)  & 0xFF);
  pkt[16] = (uint8_t)((baud >> 16) & 0xFF);
  pkt[17] = (uint8_t)((baud >> 24) & 0xFF);
  ...
  m_device->write(pkt, sizeof(pkt));

  if (waiting()) {
    ack_received     = false;
    nak_received     = false;
    ack_same_as_sent = false;
    ack_expected     = false; // CFG-PRT often invalidates the current link timing immediately
  }
...
bool ubloxGPS::parseField( char c )
{
  ...
  // Serial.printf("parseField %c\n\r",c);


  if ((nmeaMessage != NMEAGPS::NMEA_UNKNOWN) && (nmeaMessage != UBX_MSG))
    return NMEAGPS::parseField( c ); // Fallback to NMEA parser if we're in an NMEA message
  ...
  switch (rx().msg_class) {
    ...
    case UBX_NAV: //=================================================
      ...
      switch (rx().msg_id) {
        ...
        case UBX_NAV_SAT    : break; //return parseNavSatS   ( chr );
        default             : 
        
        Serial.printf("UBX_NAV unknown msg_id[0x%02X] \n\r", rx().msg_id);
        break;
      }
      break;
    case UBX_HNR: //=================================================
...
bool ubloxGPS::parseNavPvt( uint8_t chr )
{
  ...
  // Serial.printf("ubx: parseNavPvt%02X\n\r",chr);
