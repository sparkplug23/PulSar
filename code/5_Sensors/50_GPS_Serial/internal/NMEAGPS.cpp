...
// This is the main decode for this code, which is overridden by the UBX code when processing UBX packets.
...
NMEAGPS::decode_t NMEAGPS::handle( uint8_t c )
{
  ...
  // Serial.printf("BOT UBX SHOULD NOT BE HERE NMEAGPS::handle: %d\n\r", res);

  if (res == DECODE_COMPLETED) 
  {
    // Serial.printf("NMEAGPS::handle: storeFix\n\r");
    storeFix();
  } 
  else 
  if ((NMEAGPS_FIX_MAX == 0) && _available() && !is_safe()) 
  {
    // Serial.printf("NMEAGPS::handle: ELSE-------------\n\r");
...
{
  // Serial.printf("storeFix: avail_before=%d interval=%d merging=%d\n\r",
  //               _available(), intervalComplete(), merging);
  ...
  if (!room) {
    ...
    if (keepNewestFixes) {
        ...
        _fixesAvailable--;
        // Serial.print("_fixesAvailable removed 2");
  ...
  if (room) {
    ...
    if ((merging == NO_MERGING) || intervalComplete()) {
      ...
      // Serial.printf("store_fixesAvailable %d\n\r",_fixesAvailable);
...
const gps_fix NMEAGPS::read()
{
  ...
  if (_fixesAvailable) {
    lock();
      ...
      #if (NMEAGPS_FIX_MAX > 0)
        _fixesAvailable--;
        // Serial.print("_fixesAvailable removed - read should pop from here in the ideal");
