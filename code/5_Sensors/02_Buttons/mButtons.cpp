...
void mButtons::Pre_Init(void)
{
  ...
  for(uint32_t i=0;i<MAX_KEYS_SET;i++)
  {
    ...
    {
      SetButtonUsed(i);
      pin = tkr_pins->GetPin(GPIO_KEY1_ID, i);
      pinMode(pin, GetHardwareSpecificPullMethod(pin)); // Note: GPIO16/D0 inversion is pulldown, not up
    }else    
    if(tkr_pins->PinUsed(GPIO_KEY1_INV_ID, i))
    {
      SetButtonUsed(i);
      pin = tkr_pins->GetPin(GPIO_KEY1_INV_ID, i);
      pinMode(pin, GetHardwareSpecificPullMethod(pin));
      InvertFlag(i);
    }else    
    if(tkr_pins->PinUsed(GPIO_KEY1_NP_ID, i))
    {
      SetButtonUsed(i);
      pin = tkr_pins->GetPin(GPIO_KEY1_NP_ID, i);
      pinMode(pin, INPUT);
      PullupFlag(i); 
    }else    
    if(tkr_pins->PinUsed(GPIO_KEY1_INV_NP_ID, i))
    {
      SetButtonUsed(i);
      pin = tkr_pins->GetPin(GPIO_KEY1_INV_NP_ID, i);
      pinMode(pin, INPUT);
      PullupFlag(i); 
      InvertFlag(i); 
    }
    ...
    if(tkr_pins->PinUsed(GPIO_KEY1_TOUCH_ID, i))
    {
      SetButtonUsed(i);
      pin = tkr_pins->GetPin(GPIO_KEY1_TOUCH_ID, i);      
      TouchFlag(i);
    }
...
bool mButtons::Used(uint32_t index) {
  return (tkr_pins->PinUsed(GPIO_KEY1_ID, index) || bitRead(Button.used_bitmap, index));
}
...
void mButtons::Probe(void) 
{
  ...
  for (uint32_t i = 0; i < MAX_KEYS_SET; i++) {
    ...
    if (tkr_pins->PinUsed(GPIO_KEY1_ID, i)) {
      not_activated = (digitalRead(tkr_pins->GetPin(GPIO_KEY1_ID, i)) != bitRead(Button.inverted_bitmap, i));
    } else if (tkr_pins->PinUsed(GPIO_KEY1_INV_ID, i)) { // Inverted pin, active low
      not_activated = digitalRead(tkr_pins->GetPin(GPIO_KEY1_INV_ID, i));
    } else if (tkr_pins->PinUsed(GPIO_KEY1_NP_ID, i)) { // No pull, active high
      not_activated = digitalRead(tkr_pins->GetPin(GPIO_KEY1_NP_ID, i));
    } else if (tkr_pins->PinUsed(GPIO_KEY1_INV_NP_ID, i)) { // Inverted pin, no pull, active low
      not_activated = digitalRead(tkr_pins->GetPin(GPIO_KEY1_INV_NP_ID, i));
    } 
    #if defined(SOC_TOUCH_VERSION_1) || defined(SOC_TOUCH_VERSION_2)
    else
    if (tkr_pins->PinUsed(GPIO_KEY1_TOUCH_ID, i)) {
      if (bitRead(TouchButton.touch_bitmap, i)) {
        ...
        uint32_t value = touchRead( tkr_pins->Pin(GPIO_KEY1_TOUCH_ID, i));
  ...
  for (uint32_t button_index = 0; button_index < MAX_KEYS_SET; button_index++) {
    ...
    if (Used(button_index)) {
      ...
      if (bitRead(TouchButton.touch_bitmap, button_index) && bitRead(TouchButton.calibration, button_index +1)) {  // Touch
        uint32_t _value = touchRead( tkr_pins->Pin(GPIO_KEY1_TOUCH_ID, button_index) );
        #ifdef SOC_TOUCH_VERSION_2
