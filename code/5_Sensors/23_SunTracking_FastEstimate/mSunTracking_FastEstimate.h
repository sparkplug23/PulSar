...
public:
    ...
    static constexpr const char* PM_MODULE_SENSORS_SUN_TRACKING_FAST_ESTIMATE_CTR =
        "SunTracking_FastEstimate";

    PGM_P GetModuleName()
    {
        return PM_MODULE_SENSORS_SUN_TRACKING_FAST_ESTIMATE_CTR;
    }

    uint16_t GetModuleUniqueID()
    {
        return D_UNIQUE_MODULE_SENSORS_SUN_TRACKING_FAST_ESTIMATE_ID;
    }
    ...
    float Get_Elevation_Max() const
    {
        return calc.max_elevation;
    }


    float Get_Elevation_Min() const
    {
    ...
    void ScheduleDailyRebuild();

    void ProcessDailyRebuild();

    void PrepareDailyCalculation();

    void CalculateTablePoint(
        uint8_t point_index
    );

    void CommitDailyTable();
...
private:
    ...
    float GetTimezoneOffsetHours();

    static float WrapDegrees360(float angle);

    static float InterpolateAzimuth(
        float start,
        float end,
        float fraction
    );
