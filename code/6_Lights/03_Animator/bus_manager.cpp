...
//   else pix += _skip;
  uint8_t co = 0;//_colorOrderMap.getPixelColorOrder(pix+_start, _colorOrder);
...
//     }
//   }
  PolyBus::setPixelColorNew(_busPtr, _iType, pix, c, co);
}

RgbcctColor BusDigital::getPixelColorNew(uint16_t pix) {
  if (reversed) pix = _len - pix -1;
  else pix += _skip;
...
void BusPwm::setPixelColorNew(uint16_t pix, RgbcctColor c) 
{
  ...
  // pinMode(2, OUTPUT);
  // analogWrite(2,random(1024));

  // ledcWrite(2, random(1024));


  Serial.println("void BusPwm::setPixelColorNew(uint16_t pix, RgbcctColor c) ");
  ...
  switch (_type) {
    ...
    case TYPE_ANALOG_5CH: //RGB + warm white + cold white
      _data[4] = cw;
      w = ww;
      DEBUG_PRINTF("TYPE_ANALOG_5CH %d\n\r", _data[4]);
  ...
  DEBUG_PRINTF("BusPwm::setPixelColorNew _type %d, data [%d,%d,%d,%d,%d]\n\r", _type, _data[0], _data[1], _data[2], _data[3], _data[4]);


}

//does no index check
RgbcctColor BusPwm::getPixelColorNew(uint16_t pix) {
  if (!_valid) return 0;
  return RgbcctColor(_data[0], _data[1], _data[2], _data[3], _data[4]);
}
...
void BusPwm::show() {
  ...
  // tmp_colour[0] = pCONT_iLight->change8to10(output_colour.R);
  // tmp_colour[1] = pCONT_iLight->change8to10(output_colour.G);
  // tmp_colour[2] = pCONT_iLight->change8to10(output_colour.B);
  // tmp_colour[3] = pCONT_iLight->change8to10(output_colour.WC);
  // tmp_colour[4] = pCONT_iLight->change8to10(output_colour.WW); 
  ...
  Serial.println("======================================void BusPwm::show() {");
...
RgbcctColor BusOnOff::getPixelColorNew(uint16_t pix) {
  // if (!_valid) return 0;
  // return RGBW32(_data, _data, _data, _data);
}
...
RgbcctColor BusNetwork::getPixelColorNew(uint16_t pix) {
  // if (!_valid || pix >= _len) return 0;
  // uint16_t offset = pix * _UDPchannels;
  // return RGBW32(_data[offset], _data[offset+1], _data[offset+2], _rgbw ? (_data[offset+3] << 24) : 0);
}
...
int BusManager::add(BusConfig &bc) 
{
  ...
  Serial.printf("if (bus_count >= WLED_MAX_BUSSES) return -1;%d\n\r", bus_count);
