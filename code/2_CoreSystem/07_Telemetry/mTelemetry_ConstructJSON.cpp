...
{ //BuildHealth
  ...
  JBI->Start();
    ...
    // JBI->Add(PM_JSON_LOOPDELAY,      pCONT_sup->loop_delay_temp);
...
uint8_t mTelemetry::ConstructJSON_Log(uint8_t json_level){ 
  ...
  JBI->Start();
    JBI->Level_Start(PM_JSON_LOGLEVELS);
      JBI->Add(PM_JSON_SERIAL, pCONT_log->GetLogLevelNamebyID(pCONT_set->Settings.seriallog_level, buffer));
      JBI->Add(PM_JSON_SYSTEM, pCONT_log->GetLogLevelNamebyID(pCONT_set->Settings.syslog_level, buffer));
      JBI->Add(PM_JSON_WEB,    pCONT_log->GetLogLevelNamebyID(pCONT_set->Settings.weblog_level, buffer));
      JBI->Add(PM_JSON_TELNET, pCONT_log->GetLogLevelNamebyID(pCONT_set->Settings.telnetlog_level, buffer));
    JBI->Level_End();
  return JBI->End();
}
