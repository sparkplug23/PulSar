...
void mWebCamera::MQTTHandler_Init()
{
  ...
  ptr->ConstructJSON_function = &mWebCamera::ConstructJSON_State;
 ...
 * */
void mWebCamera::MQTTHandler_Set_TelePeriod()
{
  for(auto& handle:mqtthandler_list){
...
{
  for(auto& handle:mqtthandler_list){
    pCONT_mqtt->MQTTHandler_Command(*this, EM_MODULE_DRIVERS_CAMERA_WEBCAM_V4_ID, handle, id);
  }
}
