/**
 * @file mSIM800L.cpp
 * @brief Driver module for the SIMCom SIM800L GSM/GPRS cellular modem.
 *
 * The SIM800L is treated as a hardware driver rather than a pure network module.
 * It provides GSM/GPRS data connectivity, SMS, SIM status, signal diagnostics,
 * modem power control, and device-specific AT command behaviour through a shared
 * UART interface. Unlike the SIM7000G, it is not assumed to provide integrated
 * GNSS, although related variants or external designs may expose additional
 * modem-side capabilities.
 *
 * This module owns the SIM800L hardware control path, including UART setup,
 * baud-rate recovery, PWRKEY sequencing, modem readiness probing, AT command
 * coordination, and SIM800L-specific capability handling. Higher-level
 * interface modules consume these capabilities through narrow abstractions:
 *
 * - Cellular/network modules consume only the GPRS data-network transport capability.
 * - SMS/command interfaces consume parsed SMS events.
 * - Telemetry/status interfaces consume signal, SIM, battery, and modem state.
 *
 * In short:
 *   SIM800L driver   = hardware and AT-command owner
...
void mSIM800L::Telemetry_Init()
{

  struct telemetry_handler<mSIM800L>* ptr;
