...
void mHBridge::Pre_Init(void){
  ...
  // MOTOR SETTINGS (A)
  if(tkr_pins->PinUsed(GPIO_HBRIDGE_L9110_IA_ID) && tkr_pins->PinUsed(GPIO_HBRIDGE_L9110_OA_ID)){   
    pin_A_IA = tkr_pins->GetPin(GPIO_HBRIDGE_L9110_IA_ID];
    pinMode(pin_A_IA,OUTPUT);
    digitalWrite(pin_A_IA,LOW); //STOP THE MOTOR
    pin_A_IB = tkr_pins->GetPin(GPIO_HBRIDGE_L9110_OA_ID];
