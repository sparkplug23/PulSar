#include "mBuzzerBasic.h"

#ifdef USE_MODULE__DRIVERS_BUZZER_BASIC

int8_t mBuzzerBasic::Tasker(uint8_t function, JsonParserObject obj){
...
void mBuzzerBasic::Pre_Init(void)
{
...
void mBuzzerBasic::BuzzerSet(uint32_t state) {
  ...
  if (tkr_set->Settings.sysopt_drivers.bit.buzzer_freq_mode) {     // SetOption111 - Enable frequency output mode for buzzer
    ...
    //ALOG_DBG(PSTR("BUZ:tkr_pins->DigitalWrite %d %d"),Buzzer.pin,state);
...
void mBuzzerBasic::BuzzerBeep(uint32_t count, uint32_t on, uint32_t off, uint32_t tune, uint32_t mode) {
...
void mBuzzerBasic::BuzzerSetStateToLed(uint32_t state) {
...
void mBuzzerBasic::BuzzerBeep(uint32_t count) {
...
void mBuzzerBasic::BuzzerEvery100mSec(void) {
...
void mBuzzerBasic::parse_JSONCommand(JsonParserObject obj){
...
// void mBuzzerBasic::CommandSet_BuzzerBeep(uint16_t time_secs, uint8_t relay_id){
...
uint8_t mBuzzerBasic::ConstructJSON_Settings(uint8_t json_level, bool json_appending){
...
uint8_t mBuzzerBasic::ConstructJSON_State(uint8_t json_level, bool json_appending){
...
void mBuzzerBasic::Telemetry_Init()
{

  struct telemetry_handler<mBuzzerBasic>* ptr;
  ...
  ptr->ConstructJSON_function = &mBuzzerBasic::ConstructJSON_Settings;
  ...
  ptr->ConstructJSON_function = &mBuzzerBasic::ConstructJSON_State;
  telemetry_list.push_back(ptr);
