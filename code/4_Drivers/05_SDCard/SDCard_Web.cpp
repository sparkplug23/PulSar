...
void mSDCard::WebPage_Root_AddHandlers()
{
  #ifndef WEB_URI_CTR
  #define WEB_URI_CTR(X) static const char X[] =
#endif

  WEB_URI_CTR(PM_URL_FILESYSTEM_SDEDIT) "/sdedit";
...
bool mSDCard::Web_SDCardEditor_NormalisePath(String& path)
{
  path.trim();
...
String mSDCard::Web_SDCardEditor_ContentTypeFromPath(const String& path)
{
...
String mSDCard::Web_SDCardEditor_JSONEscape(const String& in)
{
  String out;
...
void mSDCard::Web_SDCardEditor_GET(AsyncWebServerRequest* request)
{
  if (!request) {
    return;
  }
  ...
  if (request->hasParam("list"))
  {
    ...
    File dir = Open(path.c_str(), FILE_READ);
  ...
  if (request->hasParam("edit"))
  {
    ...
    File file = Open(path.c_str(), FILE_READ);
 ...
 *
 * SUMMARY:
 * - Creates an empty file on sdcard.fs->
 *
 * EXPECTED:
 * - FormData:
 *     path=/file.txt
...
void mSDCard::Web_SDCardEditor_PUT(AsyncWebServerRequest* request)
{
  if (!request) {
    return;
  }
  ...
  File file = Open(path.c_str(), FILE_WRITE);
...
void mSDCard::Web_SDCardEditor_DELETE(AsyncWebServerRequest* request)
{
  if (!request) {
    return;
  }
...
void mSDCard::Web_SDCardEditor_POST_Upload(
  AsyncWebServerRequest* request,
  const String& filename,
  size_t index,
  uint8_t* data,
...
)
{
  ...
  if (!index)
  {
    ...
    request->_tempFile = Open(path.c_str(), FILE_WRITE);
  }
...
void mSDCard::Web_SDCardEditor_POST_Final(AsyncWebServerRequest* request)
{
  if (!request) {
    return;
  }
...
#endif // USE_MODULE_DRIVERS_SDCARD && USE_MODULE_NETWORK_WEBSERVER