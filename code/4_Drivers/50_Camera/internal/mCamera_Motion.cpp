...
uint32_t mCamera::SetMotionDetect(int32_t value) {
  ...
  if (!wc_motion.motion_detect){ // if turning it off...
    ...
    Picture_FreePtr(&wc_motion.frame);
    Picture_FreePtr(&wc_motion.last_motion);
    Picture_FreePtr(&wc_motion.diff);
    Picture_FreePtr(&wc_motion.mask);
    Picture_FreePtr(&wc_motion.background);
...
bool mCamera::ConvertFrame(int32_t bnum_i, int format, int scale) {
  ...
  struct PICSTORE *ps = &rt.picstore[bnum_i];
...
void mCamera::MotionLog(){
  AddLog(LOG_LEVEL_DEBUG_MORE, PSTR("CAM: motion: w:%d h:%d scale:1/%d:1/%d ms:%u val:%d br: %d triggerpoint:%d, px10000:%d"), 
    (rt.width/(1<<wc_motion.scale))/(1<<wc_motion.swscale), 
    (rt.height/(1<<wc_motion.scale))/(1<<wc_motion.swscale), 
    (1<<wc_motion.scale), 
    (1<<wc_motion.swscale), 
...
bool mCamera::_mono_write(void * arg, uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint8_t *data)
{
    ...
    if(!data){
        if(x == 0 && y == 0){
            ...
            jpeg->height = h;
            if (jpeg->poutput){
              tkr_camera->Picture_Alloc(jpeg->poutput, jpeg->width, jpeg->height, 0, PIXFORMAT_GRAYSCALE, 1);
            }
        } else {
            //write end
        }
...
static bool mCamera::_rgb_write(void * arg, uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint8_t *data)
{
    ...
    if(!data){
        if(x == 0 && y == 0){
            ...
            jpeg->height = h;
            if (jpeg->poutput){
              Picture_Alloc(jpeg->poutput, jpeg->width, jpeg->height, 0, PIXFORMAT_RGB888, 1);
            }
        } else {
            //write end
        }
...
static bool mCamera::_rgb565_write(void * arg, uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint8_t *data)
{
    ...
    if(!data){
        if(x == 0 && y == 0){
            ...
            jpeg->height = h;
            if (jpeg->poutput){
              Picture_Alloc(jpeg->poutput, jpeg->width, jpeg->height, 0, PIXFORMAT_RGB565, 1);
            }
        } else {
            //write end
        }
...
bool mCamera::convertJpegToPixels(const uint8_t *src_buf, size_t src_len, int width, int height, int scale, int format, struct PICSTORE *out){
  ...
  bool allocated = Picture_Alloc(out, width, height, 0, format, 1);
  if (!allocated){
    return false;
  }
...
void mCamera::AutoMask(){
  ...
  int width = rt.width/(1<<wc_motion.scale);
  int height = rt.height/(1<<wc_motion.scale);
...
void mCamera::DetectMotionFn(uint8_t *_jpg_buf, int _jpg_buf_len){
  ...
  if (newbuffers){
    ...
    Picture_AllocPtr(&wc_motion.frame, width, height, 0, PIXFORMAT_GRAYSCALE, WC_ALLOC_ALWAYS);
    if (!wc_motion.frame || !wc_motion.frame->allocatedLen) {
      AddLog(LOG_LEVEL_ERROR, PSTR("CAM: motion: no allocate frame"));
      Picture_FreePtr(&wc_motion.frame);
    }
    Picture_AllocPtr(&wc_motion.last_motion, scaledwidth, scaledheight, 0, PIXFORMAT_GRAYSCALE, WC_ALLOC_ALWAYS);
    ...
    if (wc_motion.enable_diffbuff){
      Picture_AllocPtr(&wc_motion.diff, scaledwidth, scaledheight, 0, PIXFORMAT_GRAYSCALE, WC_ALLOC_ALWAYS);
    ...
    if (wc_motion.enable_backgroundbuff){
      Picture_AllocPtr(&wc_motion.background, scaledwidth, scaledheight, 0, PIXFORMAT_GRAYSCALE, WC_ALLOC_ALWAYS);
    ...
    if (wc_motion.enable_mask){
      Picture_AllocPtr(&wc_motion.mask, scaledwidth, scaledheight, 0, PIXFORMAT_GRAYSCALE, WC_ALLOC_ALWAYS);
...
void mCamera::CmndWebcamConvertFrame(void){
  ...
  struct PICSTORE *ps = &rt.picstore[bnum-1];
  if (!ps->buff){
...
void mCamera::CmndWebcamSetPicture(void){
  ...
  struct PICSTORE *p = &rt.picstore[bnum-1];
  ...
  bool allocres = Picture_Alloc(p, width, height, len, format, 1);
  if (!allocres){
...
void mCamera::CmndWebcamSetMotionDetect(void) {
  ...
  switch(tkr_events->XdrvMailbox.index){
    case 1:
      ...
      break;
    case 2:
      res = rt.lenDiffLimit = tkr_events->XdrvMailbox.payload;
      break;
    case 3:
    ...
    case 8:{ // set mask feature.  must be done AFTER setting resolution or scale...
      ...
      if (!wc_motion.enable_mask){
        mSupport::AutoMutex localmutex(&WebcamMutex, "setMotionDetect", 30000);
...
void mCamera::CmndWebcamGetMotionPixels(void) {
  ...
  if (bnum > 1){
    bool res = Picture_Alloc(&rt.picstore[bnum-1], p->width, p->height, 0, p->format, WC_ALLOC_ALWAYS);
    if (res){
      memcpy(rt.picstore[bnum-1].buff, p->buff, p->len);
      p = &rt.picstore[bnum-1];
    } else {
      return;  // Command Error
    }
  }
