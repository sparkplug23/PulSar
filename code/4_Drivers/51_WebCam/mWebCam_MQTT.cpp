 ...
 * */
void mWebCam::MQTTHandler_Set_TelePeriod()
{
  for(auto& handle:mqtthandler_list){
 ...
 * */
void mWebCam::MQTTHandler_Sender(uint8_t id)
{
  for(auto& handle:mqtthandler_list){
    pCONT_mqtt->MQTTHandler_Command(*this, EM_MODULE_DRIVERS_CAMERA_WEBCAM_ID, handle, id);
  }
}
