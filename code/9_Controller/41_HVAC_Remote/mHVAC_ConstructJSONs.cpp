...
uint8_t mHVAC::ConstructJSON_HeatingRelays(uint8_t json_level){
  ...
  for(int device_id=0;device_id<settings.active_zones;device_id++){
    JBI->Level_Start_P(DLI->GetDeviceName_WithModuleUniqueID(tkr_relay->GetModuleUniqueID(), device_id, buffer, sizeof(buffer)));
      JBI->Add_FV(D_ONTIME, PSTR("\"%02d:%02d:%02d\""),  tkr_relay->relay_status[device_id].last.ontime.hour,  tkr_relay->relay_status[device_id].last.ontime.minute,  tkr_relay->relay_status[device_id].last.ontime.second);
      JBI->Add_FV(D_OFFTIME, PSTR("\"%02d:%02d:%02d\""), tkr_relay->relay_status[device_id].last.offtime.hour,  tkr_relay->relay_status[device_id].last.offtime.minute,  tkr_relay->relay_status[device_id].last.offtime.second);
      JBI->Add(D_TIME_ON "_Seconds",   tkr_relay->relay_status[device_id].time_seconds_on);
      JBI->Add(D_TIME_ON "_Mins",   tkr_relay->relay_status[device_id].time_seconds_on/60);
    JBI->Object_End();
  }
  return JBI->End();
