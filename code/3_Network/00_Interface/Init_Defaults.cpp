...
{
  return String(FPSTR(NETWORK_CONFIG_TEMPLATE));
}
...
bool mInterfaceNetwork::NetworkConfig_CreateFileFromTemplateAndParse(void)
{
  ...
  {
    ALOG_ERR(PSTR(D_LOG_NETWORK "NETWORK_CONFIG_TEMPLATE empty"));
    return false;
  }

  NetworkConfig_DebugPrint("NETWORK_CONFIG_TEMPLATE", template_json);
