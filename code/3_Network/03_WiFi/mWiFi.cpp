...
int8_t mWiFi::Tasker(uint8_t function, JsonParserObject obj)
{
  switch(function){
    case TASK_INIT:
    {
      ...
      bool quick_connect_started = false;

      #if defined(ENABLE_FEATURE_WIFI__SSID_QUICK_CONNECT_AFTER_OTA) && defined(ENABLE_FEATURE_RTC__SETTINGS)
      quick_connect_started = WiFi_QuickConnect_TryFromRTC();
      #endif

      if(quick_connect_started)
      {
        connection.seconds_to_wait_for_fresh_connection_attempt = -1;
      }
      else
      {
        #ifdef ENABLE_FEATURE_WIFI__SCAN_AND_RANK_PROFILES
        connection.seconds_to_wait_for_fresh_connection_attempt = 0;
        #else
...
static void ParseIPv4Config(JsonParserObject obj_ipv4, mWiFi::IPv4Config& ipv4)
{
  ...
  if(IPv4ArrayIsZero(ipv4.ip) && IPv4ArrayIsZero(ipv4.gw) && IPv4ArrayIsZero(ipv4.dns1) && IPv4ArrayIsZero(ipv4.dns2))
  {
    ipv4.is_static = 0;
  }
