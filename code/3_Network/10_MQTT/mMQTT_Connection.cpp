...
void MQTTConnection::MqttReconnect(void)
{
  ...
  if(!valid_config)
  {
    ALOG_ERR(PSTR(D_LOG_MQTT "MqttReconnect blocked: invalid config id=%s host=%s port=%u"),
      id,
      host_address,
      port
    );
    ...
    ALOG_ERR(PSTR(D_LOG_MQTT "  en=%u"), en);
    ALOG_ERR(PSTR(D_LOG_MQTT "  allowed=%u"), allowed);
    ALOG_ERR(PSTR(D_LOG_MQTT "  host_valid=%u host=%s"), host_address[0] ? 1 : 0, host_address);
    ALOG_ERR(PSTR(D_LOG_MQTT "  port_valid=%u port=%u"), port ? 1 : 0, port);
    ALOG_ERR(PSTR(D_LOG_MQTT "  client_name_valid=%u client_name=%s"), client_name[0] ? 1 : 0, client_name);
    ALOG_ERR(PSTR(D_LOG_MQTT "  network_client=%p"), network_client);
    ALOG_ERR(PSTR(D_LOG_MQTT "  pubsub=%p"), pubsub);
    ALOG_ERR(PSTR(D_LOG_MQTT "  retry=%u retry_counter=%u retry_start=%u"), retry, retry_counter, retry_counter_start_value);
    ALOG_ERR(PSTR(D_LOG_MQTT "  client_type=%u connected=%u flag_start_reconnect=%u"), client_type, connected, flag_start_reconnect);
    #endif
  ...
  ALOG_INF(PSTR(D_LOG_MQTT D_ATTEMPTING_CONNECTION " to \"%s:%d\""), host_address, port);
  ...
  sprintf_P(
    lwt_message_ondisconnect_ctr,
    PM_MQTT_LWT_PAYLOAD_FORMATED,
    tkr_sup->GetResetReason().c_str(),
