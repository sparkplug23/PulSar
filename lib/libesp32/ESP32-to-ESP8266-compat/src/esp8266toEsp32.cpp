...
void _analog_applyTimerConfig(int32_t timer) {
  ...
  if (ret != ESP_OK) {
    Serial.printf("PWM: ledc_timer_config %i failed ret=%i", timer, ret);
  }
  ...
  ret = ledc_timer_config(&cfg);
  if (ret != ESP_OK) {
    Serial.printf( "PWM: ledc_timer_config %i failed ret=%i", timer + MAX_TIMERS, ret);
  }
#endif
}
...
void analogWriteRange(uint32_t range, int32_t pin) {
  ...
  if (pwm_bit_num > LEDC_MAX_BIT_WIDTH || pwm_bit_num == 0) {
    Serial.printf( "PWM: range is invalid: %i", range);
    return;
  }
