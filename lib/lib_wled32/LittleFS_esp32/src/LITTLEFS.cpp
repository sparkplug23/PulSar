...
//     // New ESP32 Arduino core requires 3 arguments for open()
//     File f = File(open(path, "r", false));
// #else
//     // Older cores (or PlatformIO with older ESP32 platforms)
//     File f = open(path, "r");
// #endif
//     return (f == true);
// }
...
//     File f = open(path, "r");
// #endif
    // File f = open(path, "r"); // esp32 core 3.5
//     File f = open(path, "r", false); // esp32 core 5.3
//     return (f == true);
//     bool LITTLEFSImpl::exists(const char* path)
// {
#if defined(ESP_ARDUINO_VERSION) && (ESP_ARDUINO_VERSION >= ESP_ARDUINO_VERSION_VAL(2, 0, 0))
    // For ESP32 Arduino Core 2.0.0 and newer (e.g. 5.x PlatformIO releases)
    File f = open(path, "r", false);
#else
    // For older cores like 1.0.6 (e.g. PlatformIO espressif32@3.5.0)
    File f = open(path, "r");
#endif
    return (f == true);
