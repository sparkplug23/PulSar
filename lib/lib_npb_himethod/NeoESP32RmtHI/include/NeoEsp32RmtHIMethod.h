...
public:
    ...
    bool Initialize()
    {
        ...
        // IMPORTANT: don’t use ESP_ERROR_CHECK here if you want a bool-returning init path
        if (rmt_config(&config) != ESP_OK) {
            return false;
        }

        if (NeoEsp32RmtHiMethodDriver::Install(_channel.RmtChannelNumber,
                                            T_SPEED::RmtBit0,
                                            T_SPEED::RmtBit1,
                                            T_SPEED::RmtDurationReset) != ESP_OK) {
            return false;
        }
