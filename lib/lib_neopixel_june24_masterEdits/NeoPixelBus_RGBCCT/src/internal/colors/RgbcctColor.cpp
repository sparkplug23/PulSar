...
RgbcctColor::RgbcctColor(const RgbwwColor& color)
{
    R = color.R;
    G = color.G;
    B = color.B;
...
RgbcctColor RgbcctColor::LinearBlend(const RgbcctColor& left, const RgbcctColor& right, float progress)
{
    return RgbcctColor( 
        ...
        left.CW + ((right.CW - left.CW) * progress) 
    );
}

RgbcctColor RgbcctColor::LinearBlend(const RgbcctColor& left, const RgbcctColor& right, uint8_t progress)
{
    return RgbcctColor(left.R + (((static_cast<int32_t>(right.R) - left.R) * static_cast<int32_t>(progress) + 1) >> 8),
