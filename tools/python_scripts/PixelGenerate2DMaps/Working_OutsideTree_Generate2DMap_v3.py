...
def load_input_data(filename, use_onpixels=False):
    """Load input data from a JSON file."""
    with open(filename, "r") as f:
        input_data = json.load(f)
    
    if use_onpixels:
        # Convert "OnPixels" format into the expected [start, mid, end] rows
        on_pixels = input_data.get("OnPixels", [])
        processed_data = []
        for left, center, right in zip(on_pixels[0], on_pixels[1], on_pixels[2]):
            processed_data.append([left, center, right])
        return processed_data
    else:
        return input_data
    
def validate_triples(input_data):
    """Validate that each [left, center, right] triple is in ascending order, with row index and width count."""
    print("\nValidating triples (left, center, right):")
    for row_idx, (left, center, right) in enumerate(input_data):
        status = "Pass" if left <= center <= right else "Fail"
        width = right - left  # Calculate the width
        print(f"row {row_idx:02d} [{left}, {center}, {right}] -> {status} (width: {width})")
...
if __name__ == "__main__":
    input_file_dir = r"U:\GitHub\PulSar\tools\python_scripts"  # Specify the input JSON file location
    input_file = "input_mapping__outside_tree_2100_2024_v2.json"  # Specify the input JSON file
    ...
    # Flag to determine input format
    use_onpixels_format = True  # Set to True for "OnPixels" format, False for original format

    # Load input data
    input_data = load_input_data(os.path.join(input_file_dir, input_file), use_onpixels=use_onpixels_format)

    # Validate triples in the input data
    validate_triples(input_data)

    # Generate the matrix with optional reversal
    reverse_rows = False  # Change to False to keep the original order
