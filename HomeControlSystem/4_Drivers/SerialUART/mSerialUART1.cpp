...
void mSerialUART::init_UART1_ISR(){
  ...
  uart_isr_register(            // UART ISR handler will be attached to the same CPU core that this function is running on.
    ...
    uart_intr_handle_u1_static, // Interrupt handler function.
 ...
 **/
void IRAM_ATTR uart_intr_handle_u1_static(void *arg)
{ 
...
// uart_write_bytes(UART_NUM_0, (const char*)test_str, strlen(test_str));
  ...
  // while(rx_fifo_len)
  // {
  //   pCONT_uart->rxbuf2[i++] = UART1.fifo.rw_byte; // read all bytes
  ...
  // BaseType_t dummyval;
  // UBaseType_t res =  xRingbufferSendFromISR(pCONT_uart->settings.uart1.ringbuffer_handle, pCONT_uart->rxbuf2, pCONT_uart->urxlen2, &dummyval);
