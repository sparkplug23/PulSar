...
#define D_SA0_HIGH_ADDRESS              0b0011101 // D with SA0 high   0x1D
#define D_SA0_LOW_ADDRESS               0b0011110 // D with SA0 low or non-D magnetometer      0x1E
...
LSM303::LSM303(TwoWire *w, int addr)
{
  _wire = w;
  i2cAddress = addr; 
...
/**
 * To allow two of the same sensor on the same bus
 * */
bool LSM303::init_addressed(uint8_t i2c_addr)
{

  acc_address = i2c_addr;
  mag_address = i2c_addr;//(sa0 == sa0_high) ? 
  ...
  return true; /// assume it works, this WILL be unstable if sensor missing
...
{
  _wire->beginTransmission(acc_address);
...
byte LSM303::readAccReg(regAddr reg)
{
  ...
  _wire->write((byte)reg);
  last_status = _wire->endTransmission();
  _wire->requestFrom(acc_address, (byte)1);
  value = _wire->read();
...
{
  _wire->beginTransmission(mag_address);
  _wire->write((byte)reg);
  _wire->write(value);
  last_status = _wire->endTransmission();
}
...
byte LSM303::readMagReg(regAddr reg)
{
  ...
  _wire->write((byte)reg);
  last_status = _wire->endTransmission();
  _wire->requestFrom(mag_address, (byte)1);
  value = _wire->read();
  _wire->endTransmission();
...
void LSM303::readAcc(void)
{
  ...
  _wire->write(OUT_X_L_A | (1 << 7));
  last_status = _wire->endTransmission();
  _wire->requestFrom(acc_address, (byte)6);
...
void LSM303::readMag(void)
{
  ...
  _wire->write((_device == device_D) ? translated_regs[-OUT_X_L_M] | (1 << 7) : translated_regs[-OUT_X_H_M]);
  last_status = _wire->endTransmission();
  _wire->requestFrom(mag_address, (byte)6);
