...
void mSensorsINA219::WebAppend_Root_Status_Table_Draw(){
  ...
  BufferWriterI->Append_P(PSTR("<tr><td><b>INA219 Current Sensor</b></td></tr>"));//GetPaletteFriendlyName(),GetPixelsInMap(pCONT_iLight->palettelist.ptr));
  ...
  for(int col=0;col<settings.fSensorCount+1;col++){ //sensors + title colomn
    if(col==0){ //first column blank
      ...
      BufferWriterI->Append_P(PSTR("<td>%s</td>"), pCONT_set->GetDeviceName(D_MODULE_SENSORS_INA219_ID,col-1,buffer,sizeof(buffer)));
    }
  }    
...
uint8_t mSensorsINA219::ConstructJSON_Sensor(uint8_t json_level){
  ...
  for(uint8_t sensor_id = 0;sensor_id<MAX_SENSORS;sensor_id++){
    // if(sensor[sensor_id].ischanged_over_threshold || (json_level>JSON_LEVEL_IFCHANGED)){
      JsonBuilderI->Level_Start_P(pCONT_set->GetDeviceName(D_MODULE_SENSORS_INA219_ID,sensor_id,buffer,sizeof(buffer))); 
