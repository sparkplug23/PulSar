...
// uint16_t mAnimatorLight::candle(bool multi)
// {
//   if (multi)
//   {
//     //allocate segment data
//     uint16_t dataSize = (SEGLEN -1) *3;
//     if (!_segment_runtimes[_segment_index].allocateData(dataSize)) return candle(false); //allocation failed
//   }
...
//      if (i > 0) {
//       pCONT_lAni->mEffects->setPixelColor(i, color_blend(SEGCOLOR(1), pCONT_lAni->mEffects->color_from_palette(i, true, PALETTE_SOLID_WRAP, 0), s));

//       _segment_runtimes[_segment_index].data[d] = s; _segment_runtimes[_segment_index].data[d+1] = s_target; _segment_runtimes[_segment_index].data[d+2] = fadeStep;
//     } else {
//       for (uint16_t j = 0; j < SEGLEN; j++) {
//         pCONT_lAni->mEffects->setPixelColor(j, color_blend(SEGCOLOR(1), pCONT_lAni->mEffects->color_from_palette(j, true, PALETTE_SOLID_WRAP, 0), s));
//       }

//       _segment_runtimes[_segment_index].aux0 = s; _segment_runtimes[_segment_index].aux1 = s_target; _segment_runtimes[_segment_index].step = fadeStep;
//     }
//   }
...
// uint16_t mAnimatorLight::mode_candle()
// {
//   return candle(false);
// }


// uint16_t mAnimatorLight::mode_candle_multi()
// {
//   return candle(true);
// }
